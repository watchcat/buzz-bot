...
| `POST` | `/internal/embed` | Trigger embedding batch (called by CronJob) |
| `POST` | `/internal/embeddings_result` | Callback from RunPod embed worker |
...
users ──< user_feeds >── feeds ──< episodes ──< user_episodes >── users
                                       │
                                       ├──< episode_embeddings (pgvector + topics)
                                       ├──< dubbed_episodes
...
Hybrid system combining semantic similarity (70%) with collaborative filtering (30%):

1. **Vector search** — find 20 nearest episodes by cosine similarity using pgvector HNSW index
2. **Collaborative signal** — count users who liked both the current episode and each candidate
3. **Merge** — `FULL OUTER JOIN` both signals, weight 70/30, rank top 5
4. **Topic overlap** — compute shared KeyBERT keywords between source and recommended episodes for explainability

Embeddings: BGE-M3 (1024-dim), generated by RunPod Serverless worker. See [RECOMMENDATIONS.md](RECOMMENDATIONS.md) for details.
